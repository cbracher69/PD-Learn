...
def scatter_gauss(x_series, y_series, subj_cond):
    ...
    image_resolution_x = 0.02
    image_resolution_y = 0.02
    ...
    plt.title(x_series.name + ' vs. ' + y_series.name)
...
def profile_gauss(x_series, subj_cond, cohorts, data_counts):
    ...
    for c in cohorts:
        cdf_dict[c] = np.zeros(gridpoints)
    ...
    for subj in subj_cond.index:
        ...
        normal_cdf = norm.cdf(x, loc = x_series[subj], scale = Gauss_width)
        
        # Add it to proper cohort function:
        
        cdf_dict[subj_cond[subj]] += normal_cdf / data_counts[subj_cond[subj]]
    ...
    for member in cohorts:
        ax.plot(x, cdf_dict[member], lw = 2, c = rgb[member], label = member)
